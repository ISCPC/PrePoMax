...
namespace Octree
{
    ...
    public partial class BoundsOctree<T>
    {
        ...
        private class Node
        {
            ...
            public Node(double baseLengthVal, double minSizeVal, double loosenessVal, Point centerVal)
            {
            ...
            /// </summary>
            /// <param name="plane">Plane to check. Passing by ref as it improves performance with structs.</param>
            /// <param name="collided">List result.</param>
            /// <returns>Objects that intersect with the specified plane.</returns>
            public void GetColliding(ref Plane plane, List<T> collided)
            {
                double distance;
                ...
                for (int i = 0; i < _objects.Count; i++)
                {
                    if (_objects[i].Bounds.IntersectPlane(plane, out distance) && 
                        _objects[i].Obj is IIntersect isp && isp.Intersect(plane, out distance))
                    {
                        collided.Add(_objects[i].Obj);
                    }
                }
                ...
                if (_children != null)
                {
                    for (int i = 0; i < 8; i++)
                    {
                        _children[i].GetColliding(ref plane, collided);
                    }
                }
            }

            /// <summary>
            /// Returns an array of objects that are on one side of the specified plane, if any. Otherwise returns an empty array.
            /// </summary>
            /// <param name="plane">Plane to check. Passing by ref as it improves performance with structs.</param>
            /// <param name="positiveSide">Select the side of the plane where objects are located.</param>
            /// <param name="uncollided">List result.</param>
            /// <returns>Objects that are on one side of the specified plane.</returns>
            public void GetUncollided(ref Plane plane, bool positiveSide, List<T> uncollided)
            {
                double distance;
                // Is the plane at least partially in this node?
                if (!_bounds.IntersectPlane(plane, out distance))
                {
                    if ((positiveSide && distance > 0) || !positiveSide && distance < 0) GetAllObjects(uncollided);
                    return;
                }
                ...
                for (int i = 0; i < _objects.Count; i++)
                {
                    if (!_objects[i].Bounds.IntersectPlane(plane, out distance))
                    {
                        if ((positiveSide && distance > 0) || !positiveSide && distance < 0) uncollided.Add(_objects[i].Obj);
                    }
                    else
                    {
                        if (_objects[i].Obj is IIntersect isp && !isp.Intersect(plane, out distance))
                        {
                            if ((positiveSide && distance > 0) || !positiveSide && distance < 0) uncollided.Add(_objects[i].Obj);
                        }
                    }
                ...
                if (_children != null)
                {
                    for (int i = 0; i < 8; i++)
                    {
                        _children[i].GetUncollided(ref plane, positiveSide, uncollided);
                    }
                }
            }
            ...
            private void SetValues(double baseLengthVal, double minSizeVal, double loosenessVal, Point centerVal)
            {

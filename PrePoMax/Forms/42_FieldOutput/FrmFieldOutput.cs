...
namespace PrePoMax.Forms
{
    class FrmFieldOutput : UserControls.FrmPropertyListView, IFormBase
    {
        ...
        protected override bool OnPrepareForm(string stepName, string fieldOutputToEditName)
        {
            ...
            _stepName = stepName;
            _fieldOutputNames = _controller.GetFieldOutputNamesForStep(stepName);
            ...
            {
                FieldOutput = _controller.GetFieldOutput(stepName, fieldOutputToEditName); // to clone
        ...
        private void PopulateListOfFieldOutputs()
        {
            ...
            ViewNodalFieldOutput vnfo = new ViewNodalFieldOutput(new NodalFieldOutput(GetFieldOutputName("N"), NodalVariable.U));
            item.Tag = vnfo;
            lvTypes.Items.Add(item);

            item = new ListViewItem("Element output");
            ViewElementFieldOutput vefo = new ViewElementFieldOutput(new ElementFieldOutput(GetFieldOutputName("E"), ElementVariable.S | ElementVariable.E));
            item.Tag = vefo;
            lvTypes.Items.Add(item);
        }
